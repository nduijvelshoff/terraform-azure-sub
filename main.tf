...
resource "azapi_resource" "sub" {
  ...
  body = {
    properties = {
      displayName = provider::assert::null(var.subscription.subscription_id) ? var.subscription.name : null
      workload    = var.subscription.workload
      billingScope = provider::assert::null(var.subscription.subscription_id) ? try(var.subscription.billing_scope_id, null) != null ? var.subscription.billing_scope_id : try(
